...
### עדכון: תיקון בורר הקבצים - לא הציג מנהל קבצים מקומי
- **"טעינת קובץ מניינים" הציג רק אפליקציות ענן (Drive/Gmail) בלי אפשרות
  לבחור קובץ מקומי מהאחסון**: הבורר עבד עם `ACTION_OPEN_DOCUMENT`, שדורש
  שמנהל הקבצים יממש Storage Access Framework - חלק ממנהלי הקבצים
  המובנים במכשירים ישנים/מותאמים (כגון MIUI ישן) לא תומכים בכך, ולכן לא
  הופיעו כלל ברשימת האפשרויות. הוחלף ל-`ACTION_GET_CONTENT`, שנתמך ע"י
  כל מנהל קבצים (גם כאלה שלא מממשים SAF), עם `Intent.createChooser` כדי
  שתמיד תוצג רשימת כל האפליקציות הרלוונטיות.
